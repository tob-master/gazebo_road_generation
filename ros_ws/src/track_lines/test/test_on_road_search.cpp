...
OnRoadSearchInitializationParameters GetOnRoadSearchInitializationParameters(ros::NodeHandle &n)
{
...
TEST(BooleanCmpTest, ShouldPass){
    ...
    OnRoadSearchInitializationParameters on_road_search_init = GetOnRoadSearchInitializationParameters(n);
    OnRoadSearch* OnRoadSearcher = new OnRoadSearch(&n,on_road_search_init);
    OnRoadSearcher->SetImage(image_bird);
    OnRoadSearcher->LoadSafeDriveAreaEvaluation(safe_drive_area_evaluation_return_info);
    OnRoadSearcher->LoadValidationTables(left_line_validation_table,mid_line_in_drive_direction_table,right_line_validation_table);
    OnRoadSearcher->LoadInDriveDirectionTables(left_line_in_drive_direction_table,right_line_in_drive_direction_table);
    OnRoadSearcher->SetStartParameters(vanishing_point_start_parameters);
    OnRoadSearcher->SearchOnRoad();

    //OnRoadSearcher->DrawEvaluatedSafetyAreas(image_bird_color);

    imwrite("/home/tb/gazebo_road_generation/ros_ws/src/track_lines/test/test_images/on_road_search/results/on_road_search.png",image_bird_color);

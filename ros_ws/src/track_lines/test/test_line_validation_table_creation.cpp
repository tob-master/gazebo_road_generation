...
TEST(BooleanCmpTest, ShouldPass){
    ...
    imwrite("/home/tb/gazebo_road_generation/ros_ws/src/track_lines/test/test_images/line_validation_table_creation/results/line_validation_table_creation.png",image_bird_color);


    bool test_bool = line_validatiohn_table_creation_return_info.left_found_mid_point.size() > 0;

    ASSERT_EQ(true, test_bool);
}

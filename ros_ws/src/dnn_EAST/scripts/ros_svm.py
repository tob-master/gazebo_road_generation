...
class DetectLinesBoxes(object):
    ...
    def detect(self, score_map, geo_map, score_map_thresh=0.2, box_thresh=0.1, nms_thres=0.8):
...
net = cv2.dnn.readNet("/home/tb/gazebo_road_generation/ros_ws/src/dnn_EAST/scripts/frozen_east_text_detection.pb")
...
def imageCb(data):
	...
	#print(cv_image)
	backtorgb = cv2.cvtColor(cv_image,cv2.COLOR_GRAY2RGB)
	boxes=detect_lines.get_boxes(backtorgb)
	
    	if boxes is not None:
		image=detect_lines.draw_boxes(backtorgb,boxes)
		#print("yeeeesss")
	    	cv2.imwrite("imgkk.png", backtorgb)
	#try:
...
def listener():
	...
	image_sub = rospy.Subscriber("road_sign/image",Image, imageCb,queue_size=1)

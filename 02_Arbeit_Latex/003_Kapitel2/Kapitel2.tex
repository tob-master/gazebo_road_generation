...
Die Programmiersprache C{\tiny++} ist eine Erweiterung von C und wurde ab 1979 von Bjarne Stroustrup entwickelt. Die Möglichkeit mit C{\tiny++}  objektorientiert zu Software zu entwickeln und somit Probleme in Klassen zu abstrahieren, ermöglicht eine einfachere Einarbeitung in den Programm-Code. Dies ist damit zu begründen das Menschen gewohnt sind in Objekten zu denken. 
Vor allem die Performanzvorteile begünstigen das Nutzen von C{\tiny++}  in der Bildverarbeitung, da die Sprache es erlaubt maschinennah zu programmieren und somit alle Leistungsgrenzen der Hardware voll auszunutzen.  
C{\tiny++} ist eine Compilersprache. Die Performanz auf der Hardware ist deshalb auch abhängig von den enthaltenen Optimierungsalgorithmen des verwendeten Compilers. Der verwendete Compiler ist der auf dem Linux-Derivat Ubuntu 18.04 installierte GNU Compiler Collection kurz GCC. Der Programm dieser Arbeit ist mit der GCC-Version [X] entwickelt und kompiliert worden. Die Programmiersprache C{\tiny++} entwickelt sich fortlaufend weiter, um mit der sich schnell verändernden Computer-Technik Schritt zu halten. Der C{\tiny++}11-Standard und die Features der Standard Template Library (STL) werden in der in dieser Arbeit entwickelten Software ausgiebig verwendet, deshalb werden im Folgenden die wichtigsten Features vorgestellt.
...
Multimap-Container sind assoziative Container, in denen Elemente gespeichert werden, die aus einer Kombination eines Schlüsselwerts und eines zugeordneten Werts in einer bestimmten Reihenfolge bestehen und in denen mehrere Elemente über denselben Schlüssel verfügen können. Multimap-Container ermöglichen es Elemente mit gleichen Schlüsselpaaren mit einem iterator zu durchlaufen (equal{\tiny\_}range). Multimap-Container und Unordered{\tiny\_}multimap-Container weisen dieselbe Zeitkomplexität bei der Suche auf wie Map- und Unordered{\tiny\_}map-Container auf.
\begin{lstlisting}
...
if(it!=vec.end())
{ 
cout<<"Element "<<vec[pos]<<" gefunden auf Position: "<<pos<< endl; 
} 
\end{lstlisting}
\subsection{Minmax Element-Funktion}
\label{subsec:Minmax Element-Funktion}
Sucht das größte und niedrigste Element in einer gegebenen Sequenz.
...
cout<<"Min ist " <<*result.first <<?auf Position: ?<<min_pos<<endl:
cout<<"Max ist " <<*result.second<<?auf Position: ?<<max_pos<<endl; 
\end{lstlisting}
...
\begin{lstlisting}
Einfaches Beispiel:
void HalloWelt(){ cout<<?Hallo Welt!?<<endl; }
void(*funktion)();
funktion = HelloWorld;
OpenCV:
...
Definiert ein anonymes Funktions-Objekt direkt an dem Ort, an dem es aufgerufen wird oder kann als Argument für eine Funktion eingesetzt werden.
Eine Lambda Funktion kann einen Funktionszeiger ersetzen. Typischerweise werden sie verwendet um Code-Zeilen zu Kapseln um sie dann Algorithmen oder Asynchronen Methoden zu übergeben. In dieser Arbeit wurden Lambda-Funktionen verwendet, um STL-Funktionen zu erweitern. Im Folgenden ist ein Beispiel gegeben, um die kleinste euklidische Distanz von einem Punkt zu einem Vector-Container aus Linienpunkten zu finden.
\begin{lstlisting}
...
min_element(linien_punkte.begin(), 
            ...
            {
return (Distanz2d(vergleichs_punkt,lhs)<Distanz2d(vergleichs_punkt,rhs)); 
} 
);
\end{lstlisting}
...
Die Programmierbibliothek OpenCV in der Version [X] ist im ROS-Framework bereits integriert und steht für Open Computer Vision. OpenCV enthält Algorithmen für die Bildverarbeitung und das maschinelle Lernen und ist in C++ programmiert. OpenCV ist dabei auf die Bildverarbeitung in Echtzeit ausgelegt. Intel leitetw die Entwicklung dieser Open Source Bibliothek ein, um den Verkauf von neuer Hardware zu begünstigen. OpenCV verfügt auch über Module die es erlauben Algorithmen parallel auszuführen, um einen Performanz Steigerung zu ermöglichen. Mit der CUDA-API von OpenCV lassen sich Algorithmen auf NVIDIA-Hardware, wie deren Grafikkarten oder dem NVIDIA Jetson-Board, ausführen. Auch eine OpenCL-API ist vorhanden um Algorithmen auf Intel-, AMD- oder spezieller ASIC-Hardware zu ermöglichen. Mit OpenCV lassen sich auch kleine User-Interfaces programmieren. [EXPORTIERE ZU BIRDSEYE BESCHREIBUNG]Durch diese können zum Beispiel mit Verschiebereglern Variablen der Birdseye-Transformationsmatrizen zur Laufzeit des Programms angepasst werden.
...
\begin{center}
  ...
  \caption[DieROSKommunikationsarchitektur]%
           {\label{fig:DieROSKommunikationsarchitektur}%
           Die ROS Kommunikationsarchitektur.}
\end{center}
\end{figure}
